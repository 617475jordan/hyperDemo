...
def slidingWindowsEval(image):
    ...
    if(len(interval)>3):
        ...
        return []
    pin = np.array(pin)
    res =  searchOptimalCuttingPoint(image,pin,0,int(mid),3)

    cutting_pts = res[1]
    last =  cutting_pts[-1] + int(mid)
    ...
    for x in range(1,len(cutting_pts)):
        if x != len(cutting_pts)-1 and x!=1:
            ...
            section = image[0:36, cutting_pts[x - 1] - 3:cutting_pts[x-1]+ int(mid)]
        else:
            section = image[0:36,cutting_pts[x-1]:cutting_pts[x]]
        seg_block.append(section)
    refined = refineCrop(seg_block,int(mid)-1)
